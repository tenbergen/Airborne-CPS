...
namespace ahtextureconstants {
	...
	unsigned char const K_NUM_TEXTURES = 1;

	//FIXME: May need to change texture ID numbers
	char const * const K_GAUGE_FILENAMES[K_NUM_TEXTURES] = { "GaugeTex256.bmp" };
	enum TextureId { GAUGE_ID = 0 };

	double const K_GAUGE_SYMBOL_SIZE_PX = 16.0;
	double const K_GAUGE_SYMBOL_OFFSET = K_GAUGE_SYMBOL_SIZE_PX / 2.0;

	GlRgb8Color constexpr K_RECOMMENDATION_RANGE_POSITIVE = { 0.0f, 1.0f, 0.0f };
	GlRgb8Color constexpr K_RECOMMENDATION_RANGE_NEGATIVE = { 1.0f, 0.0f, 0.0f };

	GlRgb8Color constexpr K_RECOMMENDATION_RANGE_NEGATIVE_INVERTED = { 0.0f, 1.0f, 0.0f };
	GlRgb8Color constexpr K_RECOMMENDATION_RANGE_POSITIVE_INVERTED = { 1.0f, 0.0f, 0.0f };
